...
const TrainingGames={
  ...
  serological(ctx){
    let volume=0;
    const target=[5,10,15][Math.min(ctx.config.level-6,2)];
  ...
  equipment(ctx){
    ...
    ctx.stage.innerHTML=this.shell(event.title,event.text,`<div class="event-card"><div class="event-options">${event.opts.map((o,i)=>`<button data-i="${i}">${o}</button>`).join("")}</div></div>`);
    ctx.stage.querySelectorAll(".event-options button").forEach(b=>b.onclick=()=>+b.dataset.i===event.good?ctx.complete():ctx.penalize("sampleQuality",45,event.fail));
  },
  plate(ctx){
