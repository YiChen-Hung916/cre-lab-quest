...
const TrainingGames={
  ...
  pipette(ctx){
    ...
    const correct=roll<.20?"P10":roll<.40?"P20":roll<.70?"P200":"P1000";
    const ranges={
      P10:[1,10],
      P20:[11,20],
      P200:[21,200],
    ...
    const target=randomInt(...ranges[correct]);
    const showRecommendation=Number(ctx.config.level)<=3;
    const recommendation=showRecommendation
      ?`<small class="recommended-range">建議量程：${correct}</small>`
      :"";
      ...
      `
        <div class="lab-bench">
          <div class="bench-card task-card">
            ...
            <strong class="task-volume">${target} μL → Tube B</strong>
            ${recommendation}
          </div>
          ...
          <div class="pipette-rack">
            ${[
              {name:"P10",className:"p10",range:"0.5–10 μL"},
              {name:"P20",className:"p20",range:"2–20 μL"},
              {name:"P200",className:"p200",range:"20–200 μL"},
              {name:"P1000",className:"p1000",range:"100–1000 μL"}
            ].map(item=>`
              <button type="button" class="pipette ${item.className}" data-p="${item.name}" aria-label="選擇 ${item.name}">
        ...
        <div class="controls">
          <div class="control-group">
            ...
            <div class="digit-controls">
              ${[0,1,2,3].map(index=>`
                <div class="digit">
                  <button type="button" data-i="${index}" data-dir="1" aria-label="增加第 ${index+1} 位數">▲</button>
                  <span id="digit${index}">0</span>
                  <button type="button" data-i="${index}" data-dir="-1" aria-label="減少第 ${index+1} 位數">▼</button>
                </div>
              `).join("")}
            </div>
          </div>
          ...
          <div class="control-group">
            ...
            <div class="tip-box">
              ${Array.from({length:24},(_,index)=>`
                <button type="button" class="tip-dot" data-tip="${index}" aria-label="安裝第 ${index+1} 個 tip"></button>
              `).join("")}
            </div>
          ...
          <div class="control-group pressure-pad">
            ...
            <div class="tip-visual"><div id="tipFill" class="tip-fill"></div></div>
            <button type="button" id="plungerBtn" class="plunger">按壓吸液</button>
    ...
    const currentVolume=()=>digits[0]*1000+digits[1]*100+digits[2]*10+digits[3];

    stage.querySelectorAll(".pipette").forEach(button=>{
      button.addEventListener("click",()=>{
        stage.querySelectorAll(".pipette").forEach(item=>item.classList.remove("selected"));
    ...
    stage.querySelectorAll(".tip-dot").forEach(button=>{
      button.addEventListener("click",()=>{
        ...
        stage.querySelectorAll(".tip-dot").forEach(item=>item.classList.remove("selected"));
    ...
    plungerButton.addEventListener("click",()=>{
      if(!selected){
        ctx.penalize("safety",18,"請先選擇 pipette。");
        return;
      }
      if(!tipInstalled){
        ctx.penalize("safety",22,"請先安裝新的 tip。");
        return;
      }
      ...
      if(!liquidLoaded){
        ...
        if(!aspirationChecked){
          if(selected!==correct){
            ctx.penalize("accuracy",18,`本題較適合使用 ${correct}。`);
            ctx.penalize("sampleQuality",10,"使用不適當量程可能降低移液精準度。");
            roundIssues.push("Wrong pipette");
          }
          if(numeric!==target){
            ctx.penalize("accuracy",22,"吸取容量與任務不同。");
            ctx.penalize("sampleQuality",12,"錯誤容量可能影響樣本比例。");
            roundIssues.push("Wrong volume");
          }
        ...
        plungerButton.classList.add("active");
        status.textContent=`已吸取 ${numeric} μL，請再按一次排入 Tube B`;
      ...
      plungerButton.classList.remove("active");
      status.textContent="液體已排入 Tube B";
      plungerButton.textContent="排液完成";
      plungerButton.disabled=true;
      this.finishRound(ctx,roundIssues);
    });
  },
  ...
  serological(ctx){
    ...
    const target=[5,10,15,20,25][(ctx.config.level+ctx.config.roundIndex-5)%5];
    const issues=[];
    ctx.stage.innerHTML=this.shell("Serological Pipette 培養液轉移",`將培養液吸至 ${target} mL 刻度並排入 flask。`,
    ...
    <button id="aspirateBtn" class="btn btn-primary">長按吸液</button><button id="dispenseBtn" class="btn btn-soft">排入 flask</button></div></div>`);
    const fill=ctx.stage.querySelector("#seroFill"),value=ctx.stage.querySelector("#seroValue");
    let timer;
    const start=()=>{timer=setInterval(()=>{
      volume=Math.min(25,volume+.5);
      value.textContent=volume.toFixed(1);
      fill.style.height=`${Math.min(95,volume/25*100)}%`;
      if(volume>target+2&&!overPenaltyApplied){
        ctx.penalize("sampleQuality",12,"液體吸取超過指定刻度。");
        issues.push("Over-aspirated");
        overPenaltyApplied=true;
      }
    },90)};
    const stop=()=>clearInterval(timer);
    const btn=ctx.stage.querySelector("#aspirateBtn");
    ...
    ctx.stage.querySelector("#dispenseBtn").onclick=()=>{
      stop();
      if(Math.abs(volume-target)>1){
        ctx.penalize("accuracy",24,"吸取體積不正確。");
  ...
  centrifuge(ctx){
    const randomInt=(min,max)=>Math.floor(Math.random()*(max-min+1))+min;
    const level=Number(ctx.config.level||8);
    const round=Number(ctx.config.roundIndex||1);
    let holes=8;
    if(level>=11)holes=24;
    else if(level>=10)holes=16;
    else if(level>=9)holes=12;

    const maxPairs=holes===8?3:holes===12?4:holes===16?6:8;
    const pairCount=Math.min(maxPairs,Math.max(1,round+Math.floor((level-8)/2)));
    const tubeCount=pairCount*2;
    const speedOptions=[1000,1500,2000,2500,3000];
    const timeOptions=[3,5,7,10];
    const targetSpeed=speedOptions[randomInt(0,speedOptions.length-1)];
    const targetTime=timeOptions[randomInt(0,timeOptions.length-1)];
    const tubes=[];

    for(let pair=0;pair<pairCount;pair++){
      const volume=randomInt(1,20)*50;
      tubes.push({id:pair*2+1,pair,volume},{id:pair*2+2,pair,volume});
    }
    for(let index=tubes.length-1;index>0;index--){
      const randomIndex=randomInt(0,index);
      [tubes[index],tubes[randomIndex]]=[tubes[randomIndex],tubes[index]];
    }

    let selectedTube=null;
    let spinning=false;
    const placements={};
    const rotorHolesHtml=Array.from({length:holes},(_,index)=>{
      const angle=Math.PI*2*index/holes-Math.PI/2;
      const radius=holes>=24?43:holes>=16?41:39;
      const left=50+Math.cos(angle)*radius;
      const top=50+Math.sin(angle)*radius;
      return `<button type="button" class="rotor-hole" data-hole="${index}"
        style="left:${left}%;top:${top}%;transform:translate(-50%,-50%) rotate(${360/holes*index+90}deg)"
        aria-label="Rotor position ${index+1}">
        <span style="transform:rotate(-${360/holes*index+90}deg)">${index+1}</span>
      </button>`;
    }).join("");

    const tubeHtml=tubes.map(tube=>`
      <button type="button" class="tube-token" data-tube="${tube.id}" data-pair="${tube.pair}" data-volume="${tube.volume}">
        <strong>Tube ${tube.id}</strong><span>${tube.volume} μL</span>
      </button>`).join("");
    ...
    ctx.stage.innerHTML=this.shell(
      ...
      `<div class="centrifuge-wrap">
        <div><div class="rotor rotor-${holes}" id="centrifugeRotor">${rotorHolesHtml}</div></div>
        <div>
          <div class="bench-card" style="position:static;margin-bottom:12px;">
            <span class="kicker">任務條件</span>
            <p><strong>${targetSpeed} ×g</strong> ／ <strong>${targetTime} min</strong></p>
            <small>正對面的 tube 必須具有相同容量。</small>
          </div>
          <div class="tube-bank">${tubeHtml}</div>
        </div>
      </div>
      <div class="parameter-grid" style="margin-top:18px;">
        <label>離心力<strong><span id="gText">500</span> ×g</strong>
          <input id="gForce" type="range" min="500" max="3000" step="500" value="500">
        </label>
        <label>時間<strong id="timeText">1 min</strong>
          <input id="spinTime" type="range" min="1" max="10" step="1" value="1">
        </label>
      </div>
      <div class="controls">
        <button type="button" id="spinBtn" class="btn btn-primary btn-large">啟動離心機</button>
      </div>`
    );

    const stage=ctx.stage;
    const rotor=stage.querySelector("#centrifugeRotor");
    const gForce=stage.querySelector("#gForce");
    const spinTime=stage.querySelector("#spinTime");
    const gText=stage.querySelector("#gText");
    const timeText=stage.querySelector("#timeText");

    stage.querySelectorAll(".tube-token").forEach(button=>{
      button.addEventListener("click",()=>{
        if(button.disabled||spinning)return;
        stage.querySelectorAll(".tube-token").forEach(item=>item.classList.remove("selected"));
        button.classList.add("selected");
        selectedTube=button;
      });
    });

    stage.querySelectorAll(".rotor-hole").forEach(hole=>{
      hole.addEventListener("click",()=>{
        if(spinning)return;
        if(!selectedTube){
          ctx.penalize("safety",8,"請先選擇一支 tube。");
          return;
        }
        ...
        placements[position]={tubeId,pair,volume};
        hole.classList.add("filled");
        hole.innerHTML=`<strong style="transform:rotate(-${360/holes*position+90}deg)">${tubeId}</strong>`;
    ...
    gForce.addEventListener("input",()=>gText.textContent=gForce.value);
    spinTime.addEventListener("input",()=>timeText.textContent=`${spinTime.value} min`);

    stage.querySelector("#spinBtn").addEventListener("click",()=>{
      if(spinning)return;
      const positions=Object.keys(placements).map(Number);

      /* 必須放入所有 tube 才能啟動。 */
      if(positions.length!==tubeCount){
        ctx.penalize("safety",12,"尚有 tube 未放入 rotor。");
        return;
      }

      const issues=[];
      const balanced=positions.every(position=>{
        const oppositePosition=(position+holes/2)%holes;
        const current=placements[position];
        const opposite=placements[oppositePosition];
        return Boolean(opposite&&current.volume===opposite.volume);
      });

      /* 未配平、速度錯、時間錯都仍可啟動，只扣分。 */
      if(!balanced){
        ctx.penalize("safety",45,"離心機未配平。");
        issues.push("Not balanced");
      }
      if(Number(gForce.value)!==targetSpeed){
        ctx.penalize("sampleQuality",16,"離心速度設定錯誤。");
        issues.push("Wrong speed");
      }
      if(Number(spinTime.value)!==targetTime){
        ctx.penalize("sampleQuality",16,"離心時間設定錯誤。");
        issues.push("Wrong time");
      }

      spinning=true;
      stage.querySelector("#spinBtn").disabled=true;
  ...
  equipment(ctx){
    const events=[
      {title:"Vortex混合",text:"Sample需充分混合，但不可產生大量氣泡。",opts:["混合1秒","混合5秒","持續混合30秒"],good:1,fail:"Wrong mixing time"},
      {title:"Incubator設定",text:"請為哺乳類細胞設定培養條件。",opts:["25°C／0% CO₂","37°C／5% CO₂","42°C／10% CO₂"],good:1,fail:"Wrong incubation condition"},
      {title:"設備工作站",text:"請選擇正確流程處理細胞樣本。",opts:["顯微鏡→Vortex→Incubator","Vortex→Centrifuge→Microscope","Centrifuge→烘箱→Freezer"],good:1,fail:"Wrong equipment sequence"}
    ];
    const event=events[(ctx.config.roundIndex-1)%events.length];
    ctx.stage.innerHTML=this.shell(event.title,event.text,
      `<div class="event-card"><div class="event-options">${event.opts.map((o,i)=>`<button data-i="${i}">${o}</button>`).join("")}</div></div>`);
    const buttons=[...ctx.stage.querySelectorAll(".event-options button")];
    buttons.forEach(button=>button.onclick=()=>{
      buttons.forEach(item=>item.disabled=true);
      const correct=Number(button.dataset.i)===event.good;
      button.classList.add(correct?"answer-correct":"answer-wrong");
      if(!correct){
        ctx.penalize("accuracy",100,"題目回答錯誤，本關判定不通過。");
        this.finishRound(ctx,[event.fail]);
      }else{
        this.finishRound(ctx,[]);
      }
    });
  },

  plate(ctx){
    const targetCount=3+ctx.config.level-13, targets=[];
